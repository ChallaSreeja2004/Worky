# Worky Backend — Architecture Document
...
7. [IBM Bob Integration](#7-ibm-bob-integration)
8. [Recommendation Service](#8-recommendation-service)
9. [Widget Communication](#9-widget-communication)
10. [End-to-End Data Flow](#10-end-to-end-data-flow)
11. [Dependency Rules](#11-dependency-rules)
12. [SOLID Principles Applied](#12-solid-principles-applied)
13. [Clean Architecture Alignment](#13-clean-architecture-alignment)
...
| File | Responsibility |
|---|---|
| `models.py` | `TokenData` — the OAuth token set schema. `AuthorizationResponse` — what is returned to the desktop client after login. |
| `repository.py` | `TokenRepository` abstract interface. `InMemoryTokenRepository` for development. Concrete implementations (Redis, MongoDB) are added here without changing anything else. |
| `service.py` | Orchestrates the OAuth 2.0 PKCE flow: generates the authorization URL, exchanges the authorization code for tokens, silently refreshes expired tokens. Does not know about storage — delegates to `TokenRepository`. |
| `router.py` | HTTP endpoints: `GET /auth/login`, `GET /auth/callback`, `POST /auth/logout`. |
...
Each connector is a self-contained Python package under `app/connectors/<name>/`. It follows an identical internal structure regardless of which enterprise application it connects to.

```
app/connectors/outlook/
├── settings.py        # Connector-specific config (scopes, base URL, client ID)
├── connector.py       # OutlookConnector(BaseConnector) — orchestrator
├── graph_client.py    # Raw HTTP client — all httpx calls live here
├── fetchers/
│   ├── calendar.py    # CalendarFetcher — calls graph_client, returns raw JSON
│   └── email.py       # EmailFetcher — calls graph_client, returns raw JSON
├── normalizer.py      # Maps raw API JSON → typed Pydantic models
├── models.py          # CalendarEvent, Email, OutlookContext schemas
└── router.py          # Optional: direct connector endpoint for debugging
```

### Connector internal data flow

```
Connector.get_context(user_id, access_token)
    │
    ├── fetchers run concurrently via asyncio.gather()
    │       CalendarFetcher.fetch(access_token)  → raw dict
    │       EmailFetcher.fetch(access_token)     → raw dict
    │
    ├── Normalizer.normalize(raw_calendar, raw_emails)
    │       → OutlookContext (typed Pydantic model)
    │
    └── ConnectorResult.success(
            source="outlook",
            data=outlook_context.model_dump()
        )
```

### Connector responsibilities (what it MUST do)
- Authenticate using the provided `access_token`
- Collect data from its enterprise application
- Normalize raw API responses into typed Pydantic models
- Return a `ConnectorResult` — never raise an unhandled exception out of `get_context()`
- Handle partial failures gracefully (return `ConnectorResult.partial()`)

### Connector responsibilities (what it must NOT do)
- Call IBM Bob directly
- Call the Recommendation Service
- Communicate with the desktop widget
- Call another connector
- Manage token refresh (the AuthService handles this)
...
*See [`DECISIONS.md`](DECISIONS.md) for the detailed rationale behind each specific technology and design choice.*
