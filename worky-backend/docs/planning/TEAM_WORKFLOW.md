...
> **Prerequisites:** Read [`../TEAM_RULES.md`](../TEAM_RULES.md) and [`../architecture/ARCHITECTURE.md`](../architecture/ARCHITECTURE.md) before this document.
...
The diagram below shows how all parallel workstreams converge at the shared contracts without ever blocking each other.

```
  ┌─────────────────────────────────────────────────────────────────────┐
  │  PHASE 1 — Shared Foundation  (team together, completed first)     │
  │                                                                     │
  │  BaseConnector · ConnectorResult · WorkContext · TokenRepository   │
  │  ═══════════════════════════════════════════════════════════════   │
  │  These are the contracts. After this phase, all teams work         │
  │  independently using only what is defined here.                    │
  └─────────────────────────────────────────────────────────────────────┘
                               │
                 ┌─────────────┼────────────────────┐
                 ▼             ▼                     ▼
  ...
  │                  │  │                  │  │  + Bob Dev          │
  │  OutlookConnector│  │  SlackConnector  │  │                     │
  │  (BaseConnector) │  │  (BaseConnector) │  │  Depends on:        │
  │                  │  │                  │  │  BaseConnector      │
  │  Implements:     │  │  Implements:     │  │  ConnectorResult    │
  │  - graph_client  │  │  - slack_client  │  │  WorkContext        │
  │  - fetchers      │  │  - fetchers      │  │                     │
  │  - normalizer    │  │  - normalizer    │  │  Does NOT import    │
  │                  │  │                  │  │  outlook/ or slack/ │
  │  Zero knowledge  │  │  Zero knowledge  │  │                     │
  │  of Slack code   │  │  of Outlook code │  │  Works in parallel  │
  └────────┬─────────┘  └────────┬─────────┘  └──────────┬──────────┘
           │                     │                        │
           ...
           └─────────────────────┘                        │
                               ...
                               ▼                          │
  ┌─────────────────────────────────────────────────────┐ │
  │  Context Builder  (registered in main.py)           │◄┘
  │  asyncio.gather([outlook, slack, github, ...])      │
  │  → WorkContext.from_connector_results(...)          │
  └────────────────────────┬────────────────────────────┘
                           │
                           ▼  WorkContext
  ┌─────────────────────────────────────────────────────┐
  ...
  │  Cache + GET /api/v1/recommendations                │
  └────────────────────────┬────────────────────────────┘
                           │
                           ▼
                    Desktop Widget
...
**Why developers never block each other:**
- Outlook Dev owns `app/connectors/outlook/` entirely — no one else touches it
- Slack Dev owns `app/connectors/slack/` entirely — no one else touches it
- Context Builder Dev depends only on `BaseConnector` — not on Outlook or Slack internals
- The `ConnectorResult` shape is fixed in Phase 1 — no negotiation needed at integration time
